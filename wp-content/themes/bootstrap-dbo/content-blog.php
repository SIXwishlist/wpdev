...
tha_entry_before(); ?>
 ...
 <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php if ( is_sticky() AND is_home() ) : ?>
		<hgroup>
			<?php the_title( '<h1 class="entry-title accordion-heading"><a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion1" href="#collapse-' . $x . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'the-bootstrap' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">', '</a></h1>' ); ?>
			<h3 class="entry-format"><?php _e( 'Featured', 'the-bootstrap' ); ?></h3>
		</hgroup>
	<?php
		else :
			the_title( '<h1 class="entry-title accordion-heading"><a class="accordion-toggle"  data-toggle="collapse" data-parent="#accordion1" href="#collapse-' . $x .'" title="' . sprintf( esc_attr__( 'Permalink to %s', 'the-bootstrap' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">', '</a></h1>' );
		endif;
	...
	<?php else : ?>
	<div class="entry-content clearfix accordion-body collapse" id="collapse-<?php echo $x;?>">
